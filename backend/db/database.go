...
func (db *DB) GetUserByID(id int64) (*User, error) {
	...
	err := db.conn.QueryRow(
		"SELECT id, username, password_hash, avatar_url, created_at FROM users WHERE id = ?",
		id,
	).Scan(&user.ID, &user.Username, &user.PasswordHash, &user.AvatarURL, &user.CreatedAt)
...
func (db *DB) GetAllUsers() []*User {
	rows, err := db.conn.Query("SELECT id, username, password_hash, avatar_url, created_at FROM users")
	if err != nil {
		return []*User{}
	}
	defer rows.Close()
	...
	for rows.Next() {
		var user User
		rows.Scan(&user.ID, &user.Username, &user.PasswordHash, &user.AvatarURL, &user.CreatedAt)

...
function Messenger() {
  ...
  const [needsChatsRefresh, setNeedsChatsRefresh] = useState(false);
  ...
  // Refresh chats when WS message arrives for unknown chat
  useEffect(() => {
    if (needsChatsRefresh && currentUser) {
      loadChats();
      setNeedsChatsRefresh(false);
    }
  }, [needsChatsRefresh]);
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.type === "message") {
            ...
            // Update chat list; sort by most recent message
            setChats((prevChats) => {
              const chatIndex = prevChats.findIndex(c => c.id === data.chat_id);
              if (chatIndex === -1) {
                // Chat not in list yet — flag a reload (Bug 3)
                setNeedsChatsRefresh(true);
                return prevChats;
              }
              ...
              // Sort so the most recent chat is at the top
            ...
            // Bug 1 fix: only mark as read when the RECIPIENT read it, not the sender themselves
  ...
  const handleAvatarUpdate = (newAvatarUrl: string) => {
    setCurrentUser((prev: any) => prev ? { ...prev, avatar: newAvatarUrl } : prev);
  };

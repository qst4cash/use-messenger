    ...
    fetch(`/api/chats/${contact.id}/messages`, {
      ...
      .then((messages) => {
        const mediaMessages = messages.filter(
          (m: any) => m.file_type === "image" || m.file_type === "voice" || m.file_type === "audio" || m.file_type === "file"
        );
                ...
                >
                  ...
                  {item.file_type === "image" ? (
                    <img
                      src={item.content}
                      alt="media"
                      className="w-12 h-12 object-cover rounded shrink-0 bg-neutral-800"
                    />
                  ) : item.file_type === "video" ? (
                    <div className="w-12 h-12 rounded bg-neutral-800 flex items-center justify-center shrink-0 relative overflow-hidden">
                      <video src={item.content} className="absolute inset-0 w-full h-full object-cover opacity-60" muted />
                      <FileText className="w-5 h-5 text-neutral-300 relative z-10" />
                    </div>
                  ) : item.file_type === "voice" || item.file_type === "audio" ? (

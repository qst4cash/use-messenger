...
var upgrader = websocket.Upgrader{
	CheckOrigin: func(r *http.Request) bool {
		// Check ALLOW_ALL_ORIGINS env var (for production debugging)
		...
		// Also check ALLOWED_ORIGINS_REGEX for pattern matching
		allowedRegex := os.Getenv("ALLOWED_ORIGINS_REGEX")
		if allowedRegex != "" {
			if matched, _ := regexp.MatchString(allowedRegex, origin); matched {
				return true
			}
		}

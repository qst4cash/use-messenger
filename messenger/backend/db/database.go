...
// parseTimestamp handles the various formats SQLite may emit for DATETIME columns,
// always returning a UTC time.Time. Falls back to time.Now() if nothing matches.
func parseTimestamp(s string) time.Time {
	...
	log.Printf("parseTimestamp: unrecognised format %q, using Now", s)
	return time.Now().UTC()
}
...
func (db *DB) CreateMessage(chatID, userID int64, content string) (*Message, error) {
	...
	// Fetch the actual created_at from database
	var msg Message
	var readInt int
	var createdAt string
	err = db.conn.QueryRow(
		"SELECT id, chat_id, user_id, content, file_url, file_type, file_name, read, created_at FROM messages WHERE id = ?",
		id,
	...
	msg.Read = readInt == 1
	msg.CreatedAt = parseTimestamp(createdAt)
